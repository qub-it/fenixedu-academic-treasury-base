...
public class TestsTuitionPaymentPlanRecalculationWithTwoStatuteTypesExemptionTypePercentageMoreThan30Ects {
    ...
    public void tuitionPaymentPlanWithReservationTaxExemption() {
        ...
        DebitEntry firstDebitEntry = DebitEntry.findActive(academicTreasuryEvent).iterator().next();
        
        RegistrationTuitionService.startServiceInvocation(registration, executionYear, new LocalDate())
                .applyEnrolledEctsUnits(new BigDecimal("40.37")) //
                .applyEnrolledCoursesCount(new BigDecimal("5")) //
                .withInferedTuitionPaymentPlan() //
                .withAllInstallments() //
        ...
        assertEquals(new BigDecimal("904.28"), academicTreasuryEvent.getAmountWithVatToPay());
        assertEquals(new BigDecimal("710.52"), academicTreasuryEvent.getNetExemptedAmount());
        
        assertEquals(new BigDecimal("177.63"), academicTreasuryEvent.getNetExemptedAmount(secondInstallmentProduct));
        assertEquals(new BigDecimal("226.07"), academicTreasuryEvent.getAmountWithVatToPay(secondInstallmentProduct));

        assertEquals(new BigDecimal("177.63"), academicTreasuryEvent.getNetExemptedAmount(firstInstallmentProduct));
        assertEquals(new BigDecimal("226.07"), academicTreasuryEvent.getAmountWithVatToPay(firstInstallmentProduct));

        assertEquals(2, DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).count());

        DebitEntry secondDebitEntry = DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).filter(d -> d != firstDebitEntry).findFirst().get();

        assertEquals(2, DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).count());
        
        assertEquals(new BigDecimal("45.63"), secondDebitEntry.getNetExemptedAmount());
        assertEquals(new BigDecimal("58.07"), secondDebitEntry.getAmountWithVat());
