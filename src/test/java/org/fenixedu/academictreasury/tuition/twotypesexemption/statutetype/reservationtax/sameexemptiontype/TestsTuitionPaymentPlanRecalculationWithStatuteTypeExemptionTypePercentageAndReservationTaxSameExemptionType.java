...
public class TestsTuitionPaymentPlanRecalculationWithStatuteTypeExemptionTypePercentageAndReservationTaxSameExemptionType {
    ...
    public void tuitionPaymentPlanWithReservationTaxExemption() {
        ...
        assertEquals(new BigDecimal("703.48"), academicTreasuryEvent.getAmountWithVatToPay());
        assertEquals(new BigDecimal("496.52"), academicTreasuryEvent.getNetExemptedAmount());

        assertEquals(new BigDecimal("0.00"), academicTreasuryEvent.getAmountWithVatToPay(firstInstallmentProduct));
        assertEquals(new BigDecimal("300.00"), academicTreasuryEvent.getNetExemptedAmount(firstInstallmentProduct).setScale(2));

        assertEquals(new BigDecimal("176.74"), academicTreasuryEvent.getAmountWithVatToPay(secondInstallmentProduct));
        assertEquals(new BigDecimal("123.26"), academicTreasuryEvent.getNetExemptedAmount(secondInstallmentProduct));

        assertEquals(new BigDecimal("263.37"), academicTreasuryEvent.getAmountWithVatToPay(thirdInstallmentProduct));
        assertEquals(new BigDecimal("36.63"), academicTreasuryEvent.getNetExemptedAmount(thirdInstallmentProduct));
        
        assertEquals(new BigDecimal("263.37"), academicTreasuryEvent.getAmountWithVatToPay(fourthInstallmentProduct));        
        assertEquals(new BigDecimal("36.63"), academicTreasuryEvent.getNetExemptedAmount(fourthInstallmentProduct));
        ...
        assertTrue("Treasury exemption type used", academicTreasuryEvent.getTreasuryExemptionsSet().stream().allMatch(
                e -> List.of(usedTreasuryExemptionType1).contains(e.getTreasuryExemptionType())));
    }
