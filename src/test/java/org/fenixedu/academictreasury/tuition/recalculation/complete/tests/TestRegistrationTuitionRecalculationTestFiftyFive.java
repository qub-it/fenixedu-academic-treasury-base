...
public class TestRegistrationTuitionRecalculationTestFiftyFive {
    ...
    public void doRecalculation() {
        ...
        try {
            TreasuryExemption.create(TreasuryExemptionType.findByCode("TET1").iterator().next(), "exemption after payment",
                    new BigDecimal("36.63"), firstInstallment);
        } catch (TreasuryDomainException e) {
            if("error.DebitEntry.exemption.not.possible.due.to.closed.debit.note".equals(e.getMessage())) {
                return;
            }
        }

        throw new RuntimeException("Should throw error in creating exemption after debit note is closed");
    }
