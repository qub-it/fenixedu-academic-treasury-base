 ...
 * Result:
 *
 * The open first installment is annulled. The paid first installment is credited
 * The remaining three instalments with exemption
 *
 */

@RunWith(FenixFrameworkRunner.class)
public class TestRegistrationTuitionRecalculationTestFortyTwo {
    ...
    public void doRecalculation() {
        ...
        assertEquals(new BigDecimal("52.67"), academicTreasuryEvent.getAmountWithVatToPay(firstInstallmentProduct));
        assertEquals(new BigDecimal("7.33"), academicTreasuryEvent.getNetExemptedAmount(firstInstallmentProduct));

        CreditEntry creditEntry = firstInstallment.getCreditEntriesSet().iterator().next();

        assertEquals(new BigDecimal("110.70"), creditEntry.getAmountWithVat());
        assertEquals(new BigDecimal("15.39"), creditEntry.getNetExemptedAmount());
        ...
        assertEquals(new BigDecimal("52.67"), secondInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("7.33"), secondInstallment.getNetExemptedAmount());

        assertEquals(new BigDecimal("210.68"), academicTreasuryEvent.getAmountWithVatToPay());
        assertEquals(new BigDecimal("29.32"), academicTreasuryEvent.getNetExemptedAmount());
