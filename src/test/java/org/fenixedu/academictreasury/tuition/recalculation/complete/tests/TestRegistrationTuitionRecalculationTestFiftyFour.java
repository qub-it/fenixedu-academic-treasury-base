 ...
 * Result:
 *
 * The first instalment is annulled and credited
 * The remaining three instalments are created.
 * The new first installment is partially settled with the credit
 *
 */

@RunWith(FenixFrameworkRunner.class)
public class TestRegistrationTuitionRecalculationTestFiftyFour {
    ...
    public void doRecalculation() {
        ...
        assertEquals(new BigDecimal("0"), academicTreasuryEvent.getNetExemptedAmount());
        ...
        settlementNoteBean.getInvoiceEntryBean(firstInstallment).setSettledAmount(new BigDecimal("200"));
        settlementNoteBean.setFinantialEntity(firstInstallment.getFinantialEntity());

        settlementNoteBean.getPaymentEntries()
                .add(new SettlementNoteBean.PaymentEntryBean(new BigDecimal("200"), PaymentMethod.findByCode("NU"),
                        null));
        ...
        DebitEntry secondFirstInstallment = DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct)
                .filter(de -> de != firstInstallment).findFirst()
                .orElseThrow();
        ...
        RegistrationTuitionService.startServiceInvocation(registration, executionYear, new LocalDate())
                ...
                .withAllInstallments().forceInstallmentsEvenTreasuryEventIsCharged(true) //
        ...
        CreditEntry settledCreditEntry =
                firstInstallment.getCreditEntriesSet().stream().filter(ce -> !ce.getSettlementEntriesSet().isEmpty()).iterator()
                        .next();

        assertEquals(new BigDecimal("200.00"), settledCreditEntry.getNetAmount());
        assertEquals(new BigDecimal("0"), settledCreditEntry.getNetExemptedAmount());

        DebitEntry thirdFirstInstallment =
                DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct)
                        .filter(de -> de.getFinantialDocument().isClosed())
                        .iterator().next();

        assertEquals(true, thirdFirstInstallment != firstInstallment);
        assertEquals(true, thirdFirstInstallment != secondFirstInstallment);

        assertEquals(new BigDecimal("200.00"), thirdFirstInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("27.82"), thirdFirstInstallment.getNetExemptedAmount());
        ...
        assertEquals(new BigDecimal("263.37"), secondInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("36.63"), secondInstallment.getNetExemptedAmount());

        assertEquals(new BigDecimal("1053.48"), academicTreasuryEvent.getAmountWithVatToPay());
        assertEquals(new BigDecimal("146.52"), academicTreasuryEvent.getNetExemptedAmount());
