 ...
 * 1.º Moment:
 *
 * Student has exemption T2 100% by each installment
 *
 ...
 * 2.ª Moment:
 *
 * The exemption T2 is replaced with another T1 with 12.21% of exemption
 * Creation of 4 instalments at 40 ECTS, €10 per ECTS
 * Recalculation of the 1st instalment
 *
 * Result:
 *
 * The first installment is annulled and replaced.
 * The remaining three instalments with exemption
 *
 */

@RunWith(FenixFrameworkRunner.class)
public class TestRegistrationTuitionRecalculationTestForty {
    ...
    private static void createTuitionAllocationData() {
        ...
        TuitionAllocation.create(tuitionPaymentPlan_1T.getTuitionPaymentPlanGroup(), registration,
                tuitionPaymentPlan_1T.getExecutionYear(), firstMoment, tuitionPaymentPlan_1T, Set.of(exemptionTypeTwo));

        TuitionAllocation.create(tuitionPaymentPlan_2T.getTuitionPaymentPlanGroup(), registration,
                tuitionPaymentPlan_2T.getExecutionYear(), secondMoment, tuitionPaymentPlan_2T, Set.of(exemptionTypeOne));
    }
    ...
    public void doRecalculation() {
        ...
        assertEquals(new BigDecimal("0.00"), firstInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("300.00"), firstInstallment.getNetExemptedAmount());
        ...
        assertEquals(new BigDecimal("0.00"), creditEntry.getAmountWithVat());
        assertEquals(new BigDecimal("300.00"), creditEntry.getNetExemptedAmount());

        assertEquals(new BigDecimal("351.16"), academicTreasuryEvent.getAmountWithVatToPay(firstInstallmentProduct));
        assertEquals(new BigDecimal("48.84"), academicTreasuryEvent.getNetExemptedAmount(firstInstallmentProduct));
        ...
        assertEquals(new BigDecimal("351.16"), secondFirstInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("48.84"), secondFirstInstallment.getNetExemptedAmount());
        ...
        assertEquals(new BigDecimal("351.16"), secondInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("48.84"), secondInstallment.getNetExemptedAmount());

        assertEquals(new BigDecimal("1404.64"), academicTreasuryEvent.getAmountWithVatToPay());
        assertEquals(new BigDecimal("195.36"), academicTreasuryEvent.getNetExemptedAmount());
