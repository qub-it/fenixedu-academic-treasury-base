...
public class TestsTuitionAllocationWithExemptionPaymentPlanRecalculationWithLessThan30Ects {
    ...
    @Test
    public void recalculationWithLessThan30Ects() {
        createTuitionPaymentPlanWithAmountByEcts();
        ...
        assertEquals(new BigDecimal("351.16"), academicTreasuryEvent.getAmountWithVatToPay());

        assertEquals(new BigDecimal("87.79"),
                DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).map(DebitEntry::getAmountWithVat)
                        .reduce(BigDecimal.ZERO, BigDecimal::add));

        assertEquals(new BigDecimal("12.21"),
                DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).map(DebitEntry::getNetExemptedAmount)
                        .reduce(BigDecimal.ZERO, BigDecimal::add));

        assertEquals(new BigDecimal("87.79"), DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct)
