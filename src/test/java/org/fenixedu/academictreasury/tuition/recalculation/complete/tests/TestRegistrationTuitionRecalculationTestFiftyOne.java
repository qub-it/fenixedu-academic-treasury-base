...
public class TestRegistrationTuitionRecalculationTestFiftyOne {
    ...
    public void doRecalculation() {
        ...
        AdhocCustomer adhocCustomer =
                AdhocCustomer.create(CustomerType.findByCode("ADHOC").iterator().next(), "999999990", "Desconhecido",
                        "Desconhecido", "Desconhecido", "Desconhecido", "0000", "PT", "123456789",
                        FinantialInstitution.findAll().collect(Collectors.toList()));

        DebtAccount payorDebtAccount = adhocCustomer.getDebtAccountFor(FinantialInstitution.findAll().iterator().next());

        DebitNote.createDebitNoteForDebitEntry(firstInstallment, payorDebtAccount, debitNoteDocumentNumberSeries, new DateTime(),
