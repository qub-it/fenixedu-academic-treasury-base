...
public class TestRegistrationTuitionRecalculationTestFiftyThree {
    ...
    public void doRecalculation() {
        ...
        assertEquals(new BigDecimal("263.37"), settledCreditEntry.getNetAmount());
        assertEquals(new BigDecimal("0"), settledCreditEntry.getNetExemptedAmount());

        CreditEntry openCreditEntry =
                firstInstallment.getCreditEntriesSet().stream().filter(ce -> ce.getSettlementEntriesSet().isEmpty()).iterator()
                        .next();
        ...
        DebitEntry secondOfFirstInstallment =
                DebitEntry.findActive(academicTreasuryEvent, firstInstallmentProduct).iterator().next();

        assertEquals(true, firstInstallment != secondOfFirstInstallment);

        assertEquals(new BigDecimal("263.37"), secondOfFirstInstallment.getAmountWithVat());
        assertEquals(new BigDecimal("36.63"), secondOfFirstInstallment.getNetExemptedAmount());
